...
#include <valarray>
using namespace std;
// example:Burgers TDTS   增加CFL数输出误差   EXP2
class cfda
{
...
public:
    ...
    void border()
    {
        borderfun(u_n0), borderfun(u_n1), borderfun(u_n2), borderfun(u_n3), borderfun(u_nn);
        borderfun(f_n0), borderfun(f_n1), borderfun(f_n2), borderfun(f_n3), borderfun(f_nn);
        borderfun(L_n0), borderfun(L_n1), borderfun(L_n2), borderfun(L_n4);
    ...
    void compt_2_stage_int2()
    {
        int i, j;
        a21 = 0.76543209876543; // 0.525  0.680; //0.765;   // 1. / 210. * (93. + sqrt(2139.));
        i = n1 - nb * 2;
        border();
        while (i-- > 0) // comput L(n0)
        {
    ...
    void compt_SGLM3_line()
    {
        ...
        {
            j = i + nb;
            L_n3[j] = -1. * df_dx(f_n3, j);
            Ltx_n0[j] = -L_n3[j] * u_n3[j];
        }
    ...
    void compt_SGLM4_line()
    {
        ...
        {
            j = i + nb;
            L_n4[j] = -1. * df_dx(f_n4, j);
            Ltx_n1[j] = -L_n4[j] * u_n4[j];
        }
        i = n1 - nb * 2;
        border();
        while (i-- > 0)
    ...
    void compt_SGLM5_line()
    {
        ...
        {
            j = i + nb;
            L_n0[j] = -1. * df_dx(f_n5, j);
            Ltx_n0[j] = -L_n0[j] * u_n5[j];
        }
        i = n1 - nb * 2;
        border();
        while (i-- > 0)
...
int main()
{
    int n, oder_n = 60, begin_cell = 3200;
    double t = 2.0, CFL = 0.8; // 计算时间总长，CFL数  CFL = 0.66 1.45
    n = begin_cell + 1;
...
void comput_tt(cfda &cff1)
{
    ...
    for (int i1 = 0; i1 < 500; i1++) // 200 50
    {
        ...
        {
            if (i1 < 1)
                cff1.compt_2_stage_int2(); // cff1.compt_SGLM_int(); //

            cff1.dt = cff1.dt * 0.5; // cff1.compt_2_stage_int();
